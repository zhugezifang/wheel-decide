...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  dict.nav.menu.forEach((menu) => {
    if (menu.href === 'quadruple-bar-graph-maker') {
      return;
    } else {
      result.push(menu);
    }
  ...
  const post = await getPost('quadruple-bar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
  ...
  return (
    ...
    <iframe src="/html/Quadruple-Bar-Graph-Maker.html" allow="autoplay"  style={{ top: '0px', left: '0px',width: '100%', height: '1650px'}}></iframe>
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/quadruple-bar-graph-maker`,
      languages: {
        'en': `${url}/en/quadruple-bar-graph-maker`,
        'zh': `${url}/zh/quadruple-bar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/quadruple-bar-graph-maker`,
    }
  }
}
