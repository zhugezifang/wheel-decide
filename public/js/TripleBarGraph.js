    ...
    create(data) {
        return {
            ...
            plugins: [{
                ...
                beforeDraw: (TripleBarChart) => {
                    const ctx = TripleBarChart.canvas.getContext('2d');
                    ctx.save();
                    ctx.globalCompositeOperation = 'destination-over';
                    ctx.fillStyle = 'white';
                    ctx.fillRect(0, 0, TripleBarChart.canvas.width, TripleBarChart.canvas.height);
                    ctx.restore();
                }
            }]
        };
...
// Download module
const download = {
    TripleBarChart(TripleBarChart, format) {
        const canvas = document.getElementById('barChart');
        const link = document.createElement('a');
        link.href = canvas.toDataURL(`image/${format}`);
        link.download = `TripleBarChart.${format}`;
        link.click();
    }
};
...
function createChart() {
    ...
    if (TripleBarChart) {
        TripleBarChart.destroy();
    }

    TripleBarChart = new Chart(ctx, chartConfig.create(data));
}
...
$(document).ready(() => {
    ...
    $('#downloadPNG').click(() => download.TripleBarChart(TripleBarChart, 'png'));
    $('#downloadJPEG').click(() => download.TripleBarChart(TripleBarChart, 'jpeg'));

    // Initial TripleBarChart
    createChart();
});